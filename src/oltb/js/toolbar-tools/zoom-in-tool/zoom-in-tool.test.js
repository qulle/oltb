...
describe('ZoomInTool', () => {
    ...
    beforeAll(() => {
        ...
        jest.spyOn(ZoomInTool.prototype, 'getMap').mockImplementation(() => {
            return mockMap;
        });
    });
    ...
    it('should toggle the tool using short-cut-key [Q]', () => {
        const options = {onClicked: () => {}};
        const spyOnClicked = jest.spyOn(options, 'onClicked');
        const spyMomentary = jest.spyOn(ZoomInTool.prototype, 'momentaryActivation');

        new ZoomInTool(options);
        simulateKeyPress('keyup', window, 'Q');
